...
def main(args=None):
    ...
    parser.add_argument('--root_path', type=str,
                        default='D:\\a-pattern_recognition\\pattern\\mywork\\SequentialPointNet\\MSR_Net\\01_3DV-Action-master(base-run-version)(FPS512)(单时序流4维特征)',
                        help='preprocess folder')
    parser.add_argument('--depth_path', type=str,
                        default='C:\\Users\\Administrator\\Desktop\\LX\\paper\\dataset\\Prosessed_dataset\\3DV-Action-master\\ntu60dataset\\',
                        help='raw_depth_png')
    #################改1#############
    ...
    parser.add_argument('--save_root_dir', type=str,
                        default='D:\\a-pattern_recognition\\pattern\mywork\\SequentialPointNet\\xsub',
                        help='output folder')
    ...
    parser.add_argument('--all_framenum', type=int, default=30, help='number of action frame')
    ...
    logging.basicConfig(format='%(asctime)s %(message)s', datefmt='%Y/%m/%d %H:%M:%S',
                        filename=os.path.join(opt.save_root_dir,
                                              '(单3dp)(去2)(2局+3简运)(位编)(cs2t1)(多池1212-66)(中2)T11F20-51212832k4816test.log'),
                        level=logging.INFO)
    ...
    #################改3#############
    netR.load_state_dict(torch.load("D:\\a-pattern_recognition\\pattern\\mywork\\SequentialPointNet\\xsub\\pointnet_para_89.pth"))
    ...
    print('NTU120:{:.2%} NTU60:{:.2%}--correct number {}--all number {}===Average loss:{:.6%}'.format(
        conf_mat.trace() / conf_mat.sum(), conf_mat60.trace() / conf_mat60.sum(), conf_mat60.trace(), conf_mat60.sum(),
        loss_sigma / (i + 1) / 2))
    logging.info('{} --nepoch{} set Accuracy:{:.2%}--correct number {}--all number {}===Average loss:{}'.format('Valid',
                                                                                                                opt.nepoch,
